...
export async function sign(
  ...
  myRegistry.register(
    ...
    codecs.osmosis.gamm.v1beta1.MsgJoinPool
  );
  myRegistry.register(
    "/osmosis.gamm.v1beta1.MsgExitPool",
    codecs.osmosis.gamm.v1beta1.MsgExitPool
  );
  myRegistry.register(
  ...
  myRegistry.register(
    ...
    codecs.osmosis.gamm.v1beta1.PoolParams
  );
  myRegistry.register(
    "/osmosis.gamm.v1beta1.PoolAsset",
    codecs.osmosis.gamm.v1beta1.PoolAsset
  );
  myRegistry.register(
  ...
  myRegistry.register(
    ...
    codecs.osmosis.lockup.MsgLockTokens
  );
  myRegistry.register(
...
function parse_legacy_tx_format(jsonTx: any) {
  ...
  switch (txType) {
    case "cosmos-sdk/MsgSend":
      if (!jsonTx.msg[0].value.from_address)
        throw Error("Missing from_address in msg[0]");
      if (!jsonTx.msg[0].value.to_address)
        throw Error("Missing to_address in msg[0]");
      if (!jsonTx.msg[0].value.amount[0].amount)
        throw Error("Missing amount in msg[0] value.amount[0]");
      return {
        msg: {
          ...
          value: {
            ...
            amount: coins(
              parseInt(jsonTx.msg[0].value.amount[0].amount),
              jsonTx.msg[0].value.amount[0].denom
            ),
          },
        },
        ...
        fee: {
          amount: coins(
            parseInt(jsonTx.fee.amount[0].amount),
            jsonTx.fee.amount[0].denom
          ),
          gas: jsonTx.fee.gas,
        },
        memo: jsonTx.memo,
    ...
    case "cosmos-sdk/MsgDelegate":
      if (!jsonTx.msg[0].value.delegator_address)
        throw Error("Missing delegator_address in msg[0]");
      if (!jsonTx.msg[0].value.validator_address)
        throw Error("Missing validator_address in msg[0]");
      if (!jsonTx.msg[0].value.amount.amount)
        throw Error("Missing amount in msg[0] value.amount[0]");
      ...
      const msgDelegate: any = {
        ...
        amount: coins(
          parseInt(jsonTx.msg[0].value.amount.amount),
          jsonTx.msg[0].value.amount.denom
        )[0],
      };
      const msg = {
    ...
    case "cosmos-sdk/MsgBeginRedelegate":
      if (!jsonTx.msg[0].value.delegator_address)
        throw Error("Missing delegator_address in msg[0]");
      if (!jsonTx.msg[0].value.validator_src_address)
        throw Error("Missing validator_src_address in msg[0]");
      if (!jsonTx.msg[0].value.validator_dst_address)
        throw Error("Missing validator_dst_address in msg[0]");
      if (!jsonTx.msg[0].value.amount.amount)
        throw Error("Missing amount in msg[0] value.amount[0]");
      const from = jsonTx.msg[0].value.delegator_address;
      const memo = jsonTx.memo;
    ...
    case "cosmos-sdk/MsgTransfer":
      if (!jsonTx.msg[0].value.receiver)
        throw Error("Missing receiver in msg[0]");
      if (!jsonTx.msg[0].value.sender) throw Error("Missing sender in msg[0]");
      if (!jsonTx.msg[0].value.source_channel)
        throw Error("Missing source_channel in msg[0]");
      if (!jsonTx.msg[0].value.source_port)
        throw Error("Missing source_port in msg[0]");
      if (!jsonTx.msg[0].value.timeout_height.revision_height)
        throw Error("Missing revision_height in msg[0] value.timeout_height");
      if (!jsonTx.msg[0].value.token.amount)
        throw Error("Missing amount in msg[0] value.token[0]");
      const msgTransfer: any = {
        ...
        token: coins(
          parseInt(jsonTx.msg[0].value.token.amount),
          jsonTx.msg[0].value.token.denom
        )[0],
        timeoutHeight: {
    ...
    case "osmosis/gamm/swap-exact-amount-in":
      if (!jsonTx.msg[0].value.sender) throw Error("Missing sender in msg[0]");
      if (!jsonTx.msg[0].value.tokenIn)
        throw Error("Missing tokenIn in msg[0]");
      if (!jsonTx.msg[0].value.tokenOutMinAmount)
        throw Error("Missing tokenOutMinAmount in msg[0]");
      if (!jsonTx.msg[0].value.routes[0].poolId)
        throw Error("Missing poolId in msg[0] routes[0]");
      if (!jsonTx.msg[0].value.routes[0].tokenOutDenom)
        throw Error("Missing tokenOutDenom in msg[0] routes[0]");
      const msgSwap: any = {
        sender: jsonTx.msg[0].value.sender,
        tokenIn: coins(
          parseInt(jsonTx.msg[0].value.tokenIn.amount),
          jsonTx.msg[0].value.tokenIn.denom
        )[0],
    ...
    case "osmosis/gamm/join-pool":
      ...
      if (!jsonTx.msg[0].value.shareOutAmount)
        throw Error("Missing poolId in msg[0]");
      if (!jsonTx.msg[0].value.tokenInMaxs[0].denom)
        throw Error("Missing tokenOutMinAmount in msg[0] .denom");
      if (!jsonTx.msg[0].value.tokenInMaxs[0].amount)
        throw Error("Missing tokenOutMinAmount in msg[0] .amount");
      if (!jsonTx.msg[0].value.tokenInMaxs[1].denom)
        throw Error("Missing tokenOutMinAmount in msg[1] .denom");
      if (!jsonTx.msg[0].value.tokenInMaxs[1].amount)
        throw Error("Missing tokenOutMinAmount in msg[1] .amount");
      ...
      const msgLpAdd: any = {
        ...
        tokenInMaxs: [
          coins(
            parseInt(jsonTx.msg[0].value.tokenInMaxs[0].amount),
            jsonTx.msg[0].value.tokenInMaxs[0].denom
          )[0],
          coins(
            parseInt(jsonTx.msg[0].value.tokenInMaxs[1].amount),
            jsonTx.msg[0].value.tokenInMaxs[1].denom
          )[0],
        ],
      };
      const from = jsonTx.msg[0].value.sender;
    ...
    case "osmosis/lockup/lock-tokens":
      ...
      if (!jsonTx.msg[0].value.duration)
        throw Error("Missing duration in msg[0]");
      if (!jsonTx.msg[0].value.coins[0].denom)
        throw Error("Missing coins in msg[0] .denom");
      if (!jsonTx.msg[0].value.coins[0].amount)
        throw Error("Missing coins in msg[0] .amount");
      ...
      const msgLpStake: any = {
        ...
        coins: [
          coins(
            jsonTx.msg[0].value.coins[0].amount,
            jsonTx.msg[0].value.coins[0].denom
          )[0],
        ],
      };
